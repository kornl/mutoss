...
#' r-mean significance level equal to $\alpha$ for each group of 
...
#' $\alpha$. It follows that the concept of error rate applies 
...
#' @return list with informations about the data (sample sizes and single effects and
#' results of inferences). 
#' \item{Ordered Means}{Data frame containing the sample names (Sample),
#' sample sizes (Size), sample means (Means) and the sample variances (Variance). Here
#' the ordered means are displayed.}
#' \item{Variances}{ Data frame containg the overall variance estimator
#' (Overall) and the degreed of freedom (df).}
#' \item{SNK}{Data frame containing the All-Pairs comparisons with the comparisons described above. The table 
#' contains the mean difference
#' (Diff), its test statistic (Statistic) the adjusted p-values (Adj.P), a logical if
#' the associated hypothesis is rejected (Rejected) and the layer (Layer). If any adjusted p-value is smaller than $\alpha$
#' but this hypothesis could not be tested by hierarchical reasons, $>\alpha$ is printed.}
...
snk <- function(formula,data,alpha, MSE=NULL, df=NULL, silent = FALSE){
  ...
  #---------------- Compute helping indices ----------#
	...
	return(result)	
			...
			snk.wrapper <- function(model, data,  alpha) {
				
				result <- snk(formula=formula(model), 
						data, 
						alpha = alpha)
						difference <- cbind(result$SNK$Diff, result$SNK$Statistic, result$SNK$Layer)
						diffm<-matrix(c(difference,rep(0,length(difference)*2)),nrow=length(difference))
				rownames(diffm)<-result$SNK$Comparison
				return(list(adjPValues=result$SNK$Adj.P,rejected=result$SNK$Rejected,
